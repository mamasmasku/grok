...
// ── Rapi Dengan Text Overlay system prompt builder ────────────────────────
const buildRapiDenganTextSystemPrompt = (
  ...
  const buildSegmentTemplate = (segIdx: number): { sections: string; segMaxWords: number } => {
  ...
  const getSceneVisual = (n: number, _total: number): string => {
    if (category === 'Produk Jualan') {
      const visuals = [
        `wide shot produk terdisplay rapi dengan latar bersih, pencahayaan studio`,
      ...
      const visuals = [
        `wide shot panorama lokasi dengan landmark dan langit cerah`,
      ...
      const visuals = [
        `wide shot hidangan lengkap di meja dengan suasana outlet terlihat`,
...
LATAR BELAKANG:
• [Lingkungan/suasana spesifik dari hasil riset]
• [Elemen dekorasi, properti, atau display khas tempat]
• [Jenis pencahayaan]

ATURAN PENTING: Produk harus tampil ${categoryVisual}.

⸻
GAYA VIDEO
• [Video style utama sesuai gaya konten]
...
• SFX: [efek suara spesifik, contoh: ${sfxExample}]
...
- ATURAN NARASI MULTI-SEGMEN: Dialog MENYAMBUNG antar segmen — satu cerita utuh.
...
- SFX: deskripsi suara WAJIB spesifik.

ATURAN PANJANG DIALOG — WAJIB DIPATUHI:
Patokan: ~3 kata per detik.
...
export default function App() {
  ...
  const [promptsByMode, setPromptsByMode] = useState<Record<string, string[]>>(() => {
    try { const saved = localStorage.getItem('scriptmate_prompts'); return saved ? JSON.parse(saved) : {}; } catch { return {}; }
  });
  const [warningsByMode, setWarningsByMode] = useState<Record<string, string[][]>>(() => {
    try { const saved = localStorage.getItem('scriptmate_warnings'); return saved ? JSON.parse(saved) : {}; } catch { return {}; }
  });
  const [visualRefsByMode, setVisualRefsByMode] = useState<Record<string, string[]>>(() => {
    try { const saved = localStorage.getItem('scriptmate_visual_refs'); return saved ? JSON.parse(saved) : {}; } catch { return {}; }
  });
  ...
  const [skripJualanOutput, setSkripJualanOutput] = useState(() => {
    try { return localStorage.getItem('scriptmate_skrip_jualan') || ''; } catch { return ''; }
  });
  ...
  useEffect(() => {
    try { localStorage.setItem('scriptmate_prompts', JSON.stringify(promptsByMode)); } catch {}
  }, [promptsByMode]);

  useEffect(() => {
    try { localStorage.setItem('scriptmate_warnings', JSON.stringify(warningsByMode)); } catch {}
  }, [warningsByMode]);

  useEffect(() => {
    try { localStorage.setItem('scriptmate_visual_refs', JSON.stringify(visualRefsByMode)); } catch {}
  }, [visualRefsByMode]);

  useEffect(() => {
    try { localStorage.setItem('scriptmate_skrip_jualan', skripJualanOutput); } catch {}
  }, [skripJualanOutput]);
  ...
  const callGemini = async (body: Record<string, unknown>) => {
    const { userPrompt, systemInstruction, temperature, useSearch } = body as any;
    ...
    const toolConfig = useSearch ? { includeServerSideToolInvocations: true } : undefined;
...
- DILARANG menampilkan UI/interface aplikasi apapun di layar HP atau tablet`;
    ...
    const rapiDenganTextInstruction = buildRapiDenganTextSystemPrompt(
      category, character, segmentDuration, activeStyleGuide, stylePerContent,
      ctaType as any, totalDuration,
    );
...
${prompt}`;
  ...
  const canGenerate = !isLoading && (promptMode !== 'urai' || !!scriptInput.trim());
  ...
  const clearHistory = () => {
    if (window.confirm('Apakah Anda yakin ingin menghapus semua history?')) {
      ...
      localStorage.removeItem('scriptmate_prompts');
      localStorage.removeItem('scriptmate_warnings');
      localStorage.removeItem('scriptmate_visual_refs');
      localStorage.removeItem('scriptmate_skrip_jualan');
    }
  };
            ...
            <>
              <div className="flex flex-col gap-8">
                <div className="flex flex-col gap-4 p-6 bg-gray-800/50 border border-purple-700 rounded-xl">
                  ...
                  <ModeSelector promptMode={promptMode} setPromptMode={setPromptMode} />
                </div>
              ...
              <div className="flex flex-col gap-8">
                ...
                <div className="flex flex-col gap-4 p-6 bg-gray-800/50 border border-purple-700 rounded-xl">
                  ...
                  {promptMode === 'bebas' && (
                    <div className="bg-yellow-900/20 border border-yellow-700/50 rounded-lg px-4 py-3">
                      <p className="text-xs font-semibold text-yellow-400 mb-1">🚀 Cara Kerja Mode Ini</p>
                      <p className="text-xs text-zinc-400">Isi form → AI riset produk lewat Google lalu langsung buat prompt video sinematik gaya TikTok GO. Output berbentuk paragraf naratif siap pakai di Sora.</p>
                    </div>
                  )}
                        ...
                        <>
                          <div className="bg-green-900/20 border border-green-700/50 rounded-lg px-4 py-3">
                            ...
                            <div className="flex flex-col gap-1 text-xs text-zinc-400">
                              <p>• AI riset produk → buat <strong className="text-zinc-300">visual brief lengkap</strong></p>
                              ...
                              <p>• <strong className="text-zinc-300">Karakter hanya muncul di CTA</strong></p>
                            </div>
                          </div>
                ...
                <div className="flex flex-col gap-6 p-6 bg-gray-800/50 border border-purple-700 rounded-xl">
                  ...
                  <div className="bg-gray-900/60 border border-gray-700 rounded-lg px-4 py-3">
                    ...
                    <div className="flex flex-col gap-1 text-xs text-zinc-500">
                      ...
                      <p><span className="text-yellow-400 font-medium">Urai</span> → Membuat Prompt sora dari mengurai Narasi Skrip apa saja</p>
                ...
                {!canGenerate && !isLoading && (
                  <p className="text-xs text-center text-zinc-500 -mt-4">
                    {promptMode === 'urai' && !scriptInput.trim() ? 'Isi skrip terlebih dahulu' : ''}
                  </p>
                )}
              </div>
                ...
                <div className="flex justify-between items-center border-b border-purple-700 pb-3">
                  ...
                  {prompts.length > 0 && (
                    <div className="flex gap-2">
                      <button onClick={clearHistory} className="flex items-center gap-2 text-sm bg-red-900/50 text-red-300 px-3 py-1.5 rounded-md hover:bg-red-800/60 transition-colors border border-red-700/50">
...
function ModeSelector({ promptMode, setPromptMode }: { promptMode: string; setPromptMode: (m: PromptModeKey) => void }) {
  const modes = [
      ...
      {modes.map(({ id, label, badge }) => {
        ...
        return (
          <button key={id} onClick={() => setPromptMode(id)}
            className={`py-3 px-2 rounded-lg font-semibold transition-all text-sm leading-tight flex flex-col items-center gap-1 ${active ? 'bg-yellow-500 text-gray-900' : 'bg-gray-700/50 text-white hover:bg-gray-700'}`}>
            <span>{label}</span>
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="grid grid-cols-2 gap-3">
        <button onClick={() => onChange('tanpa-text')}
        ...
        <button onClick={() => onChange('dengan-text')}
...
function DenganTextScenePreview({ segmentDuration }: { segmentDuration: string }) {
  ...
  return (
    <div className="bg-gray-900/60 border border-green-800/60 rounded-lg px-4 py-3">
      <p className="text-xs font-semibold text-green-400 mb-3">📋 Pola skrip per segmen ({segmentDuration} detik):</p>
